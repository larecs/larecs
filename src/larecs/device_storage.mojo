...
struct DeviceStorage[*ComponentTypes: ComponentType](Copyable):
    ...
    def __init__(
        ...
        with Zone(
            function_name=(
                "DeviceStorage.__init__(var device_context: DeviceContext,"
                " length: Int)"
            )
        ):
    ...
    def __init__(out self, *, copy: Self):
        ...
        with Zone(function_name="DeviceStorage.__init__(*, copy: Self)"):
            self._columns = Self.Columns(fill=None)
    ...
    def has_component[T: ComponentType](self) -> Bool:
        ...
        """
        with Zone(
            function_name="DeviceStorage.has_component[T: ComponentType]()"
        ):
    ...
    def _create_column[T: ComponentType](mut self) raises:
        with Zone(
            function_name="DeviceStorage._create_column[T: ComponentType]()"
        ):
            comptime id = Self.component_manager.get_id[T]()
    ...
    def _copy_column[
        ...
        with Zone(
            function_name=(
                "DeviceStorage._copy_column[T: ComponentType](src:"
    ...
    def copy_to_host[T: ComponentType](self, out data: List[T]) raises:
        with Zone(
            function_name=(
                "DeviceStorage.copy_to_host[T: ComponentType](out data:"
                " List[T])"
            )
        ):
            comptime id = Self.component_manager.get_id[T]()
    ...
    def copy_to_host[
        ...
        *,
        ...
        with Zone(
            function_name=(
                "DeviceStorage.copy_to_host[T: ComponentType](column_ptr:"
                " Pointer[T, MutUntrackedOrigin], *, offset: Int, length:"
                " Int)"
            )
        ):
            comptime id = Self.component_manager.get_id[T]()
    ...
    def copy_from_host[
        ...
        with Zone(
            function_name=(
                "DeviceStorage.copy_from_host[mut: Bool, origin:"
    ...
    def get_device_ptr[
        ...
        with Zone(
            function_name="DeviceStorage.get_device_ptr[T: ComponentType]()"
        ):
            comptime id = Self.component_manager.get_id[T]()
            if self._columns[id] is None:
                raise Error("Column not initialized")
    ...
    def synchronize(self) raises:
        with Zone(function_name="DeviceStorage.synchronize()"):
            self._device_context.synchronize()
